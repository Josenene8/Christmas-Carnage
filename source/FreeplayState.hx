...
class FreeplayState extends MusicBeatState
{
	...
	{
		if (!Achievements.isAchievementUnlocked('naughty')) {
	...
	{
		var next = curDifficulty += change;

		if (!Achievements.isAchievementUnlocked('naughty') && songs[curSelected].songName == "sleigh'd" && next != 1) {
			next = 0;
		} else {
			curDifficulty = next;
		}
	...
	function changeSelection(change:Int = 0)
	{
		...
		*/ 
		...
		//who tf commits this shit? -lunar
